...
class PerceptionModule:
    ...
    def __init__(self, node: Node):
        ...
        self._last_camera_time: Optional[float] = None  # 카메라 마지막 수신 시각
    ...
    def _camera_callback(self, msg: Image) -> None:
        ...
        self._last_camera_time = time.monotonic()  # T05: 카메라 heartbeat 갱신
...
class PerceptionNode(Node):
    ...
    def _publish_crop_row(self) -> None:
        ...
        T05: 카메라 타임아웃 시 발행 중단 → planning watchdog 발화 유도
        """
        # T05: 카메라 heartbeat 확인 — 타임아웃이면 발행 중단
        last_t = self.perception._last_camera_time
        if last_t is not None:
            elapsed = time.monotonic() - last_t
            if elapsed > self._camera_timeout_sec:
                self.get_logger().warn(
                    f'[인지] 카메라 타임아웃 ({elapsed:.1f}s > '
                    f'{self._camera_timeout_sec}s) — crop_row 발행 중단'
                )
                return
