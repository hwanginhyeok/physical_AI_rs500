...
| 파일 | 설명 |
|------|------|
| `agricultural_field.sdf` | 농경지 월드 (200m x 200m). heightmap 지형, 고랑, 농로, 창고, 나무, 돌 |
...
| `package.xml` | ROS2 패키지 메타데이터 (v0.1.0, ament_python) |
...
| `perception.py` | 인지 모듈. 카메라(Image), LiDAR(PointCloud2), GPS(NavSatFix), IMU(Imu) 구독 및 데이터 저장 |
| `planning.py` | 판단 모듈. DrivingMode(IDLE/LANE_KEEPING/OBSTACLE_AVOIDANCE/EMERGENCY_STOP), 속도/조향 결정 |
| `control.py` | 제어 모듈. PID 제어기, 목표 속도/조향 → cmd_vel(Twist) 변환 |
